...
- (void)viewDidLoad
{
   ...
   self.title = @"NOC List";
    ...
    //**Alternate Way(s) with alloc] init]; or new];
    //
...
- (void)loadNocList
{
    ...
    if ([agents count]>0){
        ...
        for (int i = 0; i <= agents.count - 1 ; i+= 1){
    ...
    for(NSDictionary *anAgentDictionary in agents) {
        ...
        Agent* anAgent = [Agent agentWithDictionary:anAgentDictionary];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    //
    if ([[segue identifier] isEqualToString:@"segueIdenifier"])
    {
        //
